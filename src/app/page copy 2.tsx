...
const Button: React.FC<ButtonProps> = ({ button, onClick }) => (
  <button
    className={`rounded-[20px] border-[1px] p-[8px_16px] gap-[10px]  ${button.isClicked ? 'bg-[#1B1E1F] text-transparent bg-clip-text bg-gradient-to-r from-blue-100 via-blue-300 to-blue-500 text-white' : 'bg-[#1B1E1F] border border-[#2C3032]'}`}
    onClick={onClick}
    style={button.isClicked ? {
      background: '#1B1E1F',
      borderImage: 'rounded-[20px] border-[1px] linear-gradient(to right, #a1c4fd, #c2e9fb) 1',
    } : {}}
  >
    {button.label}
  </button>
    ...
    <>
      <div className="grid place-items-left absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 font-['WantedSans']">
        <div>
          ...
          <h3 className="font-normal text-[15px] leading-[21px] tracking--1 text-[#787B83]">원하는 설명 키워드를 모두 골라주세요</h3>
